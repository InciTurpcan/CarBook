...
namespace CarBook_WebUI.Controllers
{
    public class BlogController : Controller
    {
        ...
        public async Task<IActionResult> BlogDetails(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage2 = await client.GetAsync($"https://localhost:7117/api/Comments/CommentCountByBlog?id=" + id);
        ...
        public async Task<IActionResult> AddComment(CreateCommentDto createCommentDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7117/api/Comments/CreateCommentWithMediator", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
